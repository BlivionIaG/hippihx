...
| High-M prefill | extras @ `1046782`: **one** kernel `gptq_gemm_rdna2_prefill`. `select_config` uses ConfigA (`M_TILE=16`, `N_TILE=1024`, `THREADS=256`) for `M>256` & `N>=4096`, else ConfigC. AWQ is `use_v2_format` / `zero_offset=0`. Dest **deleted** `q_gemm_rdna2_awq_prefill.cu`. |
